// Package timezones provides the list of IANA timezone identifiers supported
// by this binary. The list is derived from the canonical IANA timezone database
// and filtered at startup against the embedded tzdata (imported via
// time/tzdata in main), so every name returned is guaranteed to be loadable
...
var all = []string{
	...
	"Etc/GMT", "Etc/GMT+0", "Etc/GMT+1", "Etc/GMT+2", "Etc/GMT+3", "Etc/GMT+4",
...
// Available is the subset of all that is loadable by time.LoadLocation with
// the embedded tzdata. Computed once at package init.
